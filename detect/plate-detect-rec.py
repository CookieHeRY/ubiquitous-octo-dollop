...
from plate_rec import get_plate_result,init_model,cv_imread
...
def order_points(pts):
    # 参考：https://blog.csdn.net/qq_51491920/article/details/124673469
    # 一共4个坐标点
...
def my_nms(dets, iou_thresh):  # nms操作
    y = dets.clone()
    y_box_score = y[:, :5]
...
def det_rec_plate(img, img_ori, detect_model, plate_rec_model):
    ...
    for output in outputs:
        ...
        # print(roi_img.shape)
        # landmarks = order_points(roi_img)
        # roi_img = four_point_transform(roi_img, landmarks)
        # landmarks = xy2xyxy(output[:4])
        # roi_img = four_point_transform(roi_img, landmarks)
        # cv2.imshow('image',roi_img)

        plate_number = get_plate_result(roi_img, device, plate_rec_model)
...
def draw_result(orgimg, dict_list):  # 车牌结果画出来
    result_str = ""
...
if __name__ == "__main__":
    ...
    parser.add_argument('--detect_model', nargs='+', type=str, default='../train/runs/detect/30000base-300epch/weights/best.pt',
                        help='model.pt path(s)')  # yolov8检测模型
    parser.add_argument('--rec_model', type=str, default='../weights/best_LPRNet_model.pth',
                        help='model.pt path(s)')  # 车牌字符识别模型
    parser.add_argument('--image_path', type=str, default='../images-yolo', help='source')  # 待识别图片路径
    parser.add_argument('--img_size', type=int, default=640, help='inference size (pixels)')  # yolov8 网络模型输入大小
    parser.add_argument('--output', type=str, default='result3', help='source')  # 结果保存的文件夹
